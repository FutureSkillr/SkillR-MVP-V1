...
export const CoachCard: React.FC<CoachCardProps> = ({ coach, selected, onSelect }) => {
  return (
    <button
      onClick={() => onSelect(coach.id)}
      className={`glass rounded-2xl p-3 sm:p-5 text-left space-y-2 sm:space-y-3 hover:scale-[1.03] active:scale-95 transition-all border group ${
        selected
    ...
    >
      ...
      <p className="text-xs sm:text-sm text-slate-400 leading-relaxed line-clamp-2">{coach.tagline}</p>
      <p className="text-[10px] sm:text-xs text-slate-500 hidden sm:block">{coach.setting}</p>
      {!selected && (
        <div className={`text-xs font-semibold ${coach.colorClass} sm:opacity-0 sm:group-hover:opacity-100 transition-opacity flex items-center gap-1`}>
          Waehlen
